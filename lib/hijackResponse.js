...
function createBrokenPromise() {
  let _resolve = () => {
    throw new Error("BrokenPromise: resolve not yet bound.");
  };
  let _reject = () => {
    throw new Error("BrokenPromise: reject not yet bound.");
  };
  ...
  brokenPromise.resolve = (...args) => _resolve(...args);
  brokenPromise.reject = (...args) => _reject(...args);
...
module.exports = function hijackResponse(res, cb) {
  ...
  const brokenPromise = createBrokenPromise();
